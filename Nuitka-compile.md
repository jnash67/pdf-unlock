# Compile to executable with Nuitka

## Overview

In general Nuitka is much easier to use than pyinstaller but it's one file mode is problematic.

## .gitignore
For a git project you want to add the following to the .gitignore file if you're using pyinstaller

```gitignore
# Nuitka compilation outputs
*.build/
*.dist/
*.onefile-build/
*.exe
...
*.dbg.c
*.dbg.h
.nuitka_battery_updated
.nuitka_python_version
```
...
```bash
# This creates a binary called __main__.bin as well as a sub-directory called __main__.build.
python -m nuitka __main__.py

# This creates a binary called PDF-Password-Remover as well as a sub-directory called __main__.build.
# You can run the executable without the .build directory but it still depends on CPython and
# any C extension modules being present on the target system 
python -m nuitka --output-filename=PDF-Password-Remover __main__.py

# This creates a PDF-Password-Remover.dist folder which you can put anywhere else and run PDF-Password-Remover.bin
# This is very very slow, especially with complex dependiencies like PyMuPDF.
python -m nuitka --standalone --output-filename=PDF-Password-Remover --enable-plugin=tk-inter __main__.py

# This is very very slow, especially with complex dependiencies like PyMuPDF.
python -m nuitka --output-filename=PDF-Password-Remover --onefile __main__.py
python -m nuitka --output-filename=PDF-Password-Remover --onefile --enable-plugin=tk-inter __main__.py
...
## Nuitka All Help --help
python -m nuitka --help
Usage: python -m nuitka [--mode=compilation_mode] [--run] [options] main_module.py
